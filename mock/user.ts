...
const CheckUser = (username : string, password : string) : UserInfo|undefined => {
  for (let u of FakeUsersInfo) {
    let _u = u as UserInfo;
    if (_u.username === username && _u.password === password) {
      return _u;
    }
  }
...
const GetUserDetail = (username: string) : UserDetail|undefined => {
  for (let u of FakeUsersDetail) {
    let _u = u as unknown as UserDetail;
...
export default {
  ...
  'GET /api/currentUser': (req: Request, res: Response) => {
    let currentUser = GetUserDetail(current_username);
    if (!getAccess() || currentUser === undefined) {
      res.status(401).send({
        data: {
          isLogin: false,
        },
